...
let universities = [];
let uni = graduates.forEach(function (item) {
  let uniNames = item.university;
  universities.push(uniNames);
});
...
const contactInfo = [];
let gradNames = graduates.forEach((item) => {
  let nameEmail = item.first_name + " " + item.email;
...
*/
const displayNames = [];
const animals = zooAnimals.forEach((item) => {
  let names = `Name: ${item.animal_name}, Scientific: ${item.scientific_name}.`;
...
const lowCase = zooAnimals.map((item) => {
  ...
  let newArr = lowCaseAnimalNames.push(lower);
  return newArr
})
...
const totPop = zooAnimals.reduce((accumulator, currentValue) => {
  let popTot = accumulator + currentValue.population;
  let tot = populationTotal + popTot;
  return tot;
}, 0)
console.log(totPop);
